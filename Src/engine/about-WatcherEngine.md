...
└──────────────────────────────────────────────────────────────────┘
                             │
┌─ STEP 3 ─ Risk guard (Trailing SL / Hard SL) ────────────────────┐
│ _manage_trailing_stop(price) → set self._sl_triggered ถ้า hit
└──────────────────────────────────────────────────────────────────┘
                             │
┌─ STEP 4 ─ Parse OHLCV + compute indicators ──────────────────────┐
│ closes = [...] from _raw_ohlcv (DataFrame/list-of-dict/list-of-list)
│ rsi  = ti["rsi"]["value"]
...
│   • Holding gold:
│       Case 1 — SL hit / price < sl_level
│                → _is_fake_swing()  → ถ้าหลอกก็ถือต่อ
│                → _is_real_reversal() (closed bar t-1, score ≥ 4/6)
│                → ถ้า real → ปลุก AI ขายฉุกเฉิน
...
| ไฟล์ | บทบาท |
|---|---|
| `Src/engine/engine.py` | สมองและ loop ของ WatcherEngine — ประกาศ `WatcherConfig` (Pydantic), `TriggerState` (cooldown + price-step lock), `WatcherEngine` (loop + strategy + trailing stop + emergency sell) |
