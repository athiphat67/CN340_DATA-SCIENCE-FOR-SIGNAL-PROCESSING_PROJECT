...
import { Globe, Coins, Activity, TrendingUp, Loader2, Radio, AlertTriangle } from 'lucide-react';

export const MarketSection = () => {
  // ดึงข้อมูลและ State ต่างๆ มาจาก Hook
  const { 
    ohlcData, 
    usdThbData, 
    techData, 
  ...
  // ⚙️ 1. การตั้งค่ากราฟแท่งเทียน XAU/THB (ใช้ useMemo เพื่อไม่ให้กราฟกระตุกเวลาหน้าจออัปเดต)
  const candlestickOptions: any = useMemo(() => ({
    chart: {
      type: 'candlestick',
  ...
  // ⚙️ 2. การตั้งค่ากราฟเส้น USD/THB (ใช้ useMemo)
  ...
  return (
    <section className="w-full min-h-screen pb-12 relative overflow-hidden" style={{ background: '#FCFBF7' }}>
      ...
      <div className="px-6 mt-12 relative z-20 max-w-7xl mx-auto">
          ...
          {!isLoading && !error && currentStats && (
            <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-gray-100 shadow-sm">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-gray-600">
                {currentStats.market_status || (currentStats.is_weekend ? 'Market Closed' : 'Market Open')}
              </span>
            </div>
          )}
        </div>
        ...
        {/* 🔴 Error State */}
        {error ? (
           <div className="flex flex-col items-center justify-center h-[50vh] bg-rose-50/50 rounded-3xl border border-rose-100 text-rose-500">
             <AlertTriangle size={48} className="mb-4 text-rose-400" />
             <p className="text-lg font-black tracking-tight mb-2">Failed to load market data</p>
             <p className="text-sm font-medium opacity-80">{error}</p>
             <button 
               onClick={() => window.location.reload()} 
               className="mt-6 px-6 py-2.5 bg-rose-500 text-white text-xs font-bold uppercase tracking-widest rounded-full hover:bg-rose-600 transition-colors shadow-lg shadow-rose-200"
             >
                Retry Connection
             </button>
           </div>
          ...
          <div className="flex flex-col items-center justify-center h-[60vh] text-[#824199]">
            ...
            <p className="text-sm font-bold uppercase tracking-widest text-gray-500">Syncing with Database...</p>
          </div>

        ) : (
          
          /* 🟢 Success State: แสดงข้อมูลเมื่อโหลดเสร็จ */
          <div className="flex flex-col gap-6 animate-in fade-in duration-500">
            
            {/* 🟢 Top Row: Live Stats */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <StatCard 
                title="Thai Gold (Sell)" 
                value={currentStats?.current_thb ? `${currentStats.current_thb.toLocaleString()}` : '-'} 
                unit="THB/g" 
                icon={<Coins size={18} />} 
                color="emerald" 
              />
              <StatCard 
                title="Global Spot" 
                value={currentStats?.current_usd ? `$${currentStats.current_usd.toLocaleString()}` : '-'} 
                unit="/ oz" 
                icon={<Globe size={18} />} 
                color="blue" 
              />
              <StatCard 
                title="Exchange Rate" 
                value={currentStats?.usd_thb ? `${currentStats.usd_thb.toFixed(2)}` : "-"} 
                unit="THB/USD" 
            ...
            <div className="bg-white p-2 rounded-[32px] border border-gray-100 shadow-sm">
              ...
              <Chart 
                ...
                series={[{ name: 'XAU/THB', data: ohlcData || [] }]} 
                type="candlestick" 
                height={450} 
              />
            </div>
            ...
            <div className="bg-white p-2 rounded-[32px] border border-gray-100 shadow-sm mt-2">
              <Chart 
                options={lineOptions} 
                series={[{ name: 'USD/THB', data: usdThbData || [] }]} 
                type="area" 
                height={250} 
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
              ...
              <div className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm relative">
                ...
                <div className="h-[200px] w-full mt-2">
                  <Chart 
                    type="area"
                    height={200}
                    series={[{ name: 'BB %B', data: techData ? techData.map((d: any) => ({ x: d.x, y: d.bb_pct_b })) : [] }]}
                    options={{
              ...
              <div className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-[9px] font-bold text-gray-400 bg-gray-50 px-2 py-1 rounded-md border border-gray-100">THB/GRAM SPREAD</span>
                </div>
                ...
                <div className="h-[200px] w-full mt-2">
                  <Chart 
                    type="bar"
                    height={200}
                    series={[{ name: 'ATR (THB)', data: techData ? techData.map((d: any) => ({ x: d.x, y: d.atr_thb })) : [] }]}
                    options={{
                      chart: { toolbar: { show: false }, background: 'transparent', animations: { enabled: false } },
                      colors: ['#f97316'],
...
const StatCard = ({ title, value, unit, icon, color, isText = false }: any) => {
  ...
  return (
    <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between h-[120px] transition-all hover:shadow-md">
      <div className="flex items-center gap-2">
