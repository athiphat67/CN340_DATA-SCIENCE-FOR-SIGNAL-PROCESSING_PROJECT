    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 px-8">
                ...
                <nav className="flex items-end justify-between border-b border-white/5 w-full font-sans pb-3">
                        ...
                        {navTabs.map((tab) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative text-sm md:text-base font-medium transition-all duration-300 whitespace-nowrap pb-1 ${
                                        isActive ? 'text-white' : 'text-white/40 hover:text-white/70'
                                    }`}
                                >
                                    {tab.name}
                                    {isActive && (
                                        <div className="absolute -bottom-[13px] left-0 right-0 h-[2px] bg-[#f9d443] shadow-[0_-2px_8px_rgba(249,212,67,0.5)]" />
                                    )}
                                </button>
                            );
                        })}
                    ...
                    {/* ฝั่งขวา: ปุ่ม AI Analysis (ปุ่มทองลอยๆ) */}
                    <div className="pl-6 pb-1">
                        <button 
                            onClick={() => navigate('/agent-analysis')}
                            className="relative inline-flex items-center justify-center px-6 py-2.5 text-sm font-bold text-[#1a0a24] bg-gradient-to-r from-[#f9d443] to-[#f9a826] rounded-full shadow-[0_6px_20px_-4px_rgba(249,212,67,0.4)] hover:shadow-[0_10px_25px_-4px_rgba(249,212,67,0.6)] hover:-translate-y-1 transition-all duration-300 group overflow-hidden whitespace-nowrap"
                        >
                            {/* เอฟเฟกต์แสงวิ่งพาดปุ่มตอนโฮเวอร์ */}
