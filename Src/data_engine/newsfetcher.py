...
class GoldNewsFetcher:
        ...
        try:
            ...
            if hasattr(ticker, "get_news"):
                try:
                    ...
                    if news:
                        logger.debug(
                            f"[{ticker_symbol}] get_news() → {len(news)} items"
                        )
                        return news
                except Exception as e:
    ...
    def _parse(raw: dict, ticker: str, category: str) -> Optional[NewsArticle]:
        ...
        if pub_str:
            try:
                # แปลงจาก String เป็นเวลาไทย
                dt = pd.to_datetime(pub_str)
                if dt.tz is None:
                    dt = dt.tz_localize("UTC")
                published_at = dt.tz_convert("Asia/Bangkok").isoformat()
            except Exception:
                published_at = pub_str
        else:
                ...
                if article and article.url not in seen_urls:
                    if category == "usd_thb":
                        thai_keywords = [
                            ...
                            "bangkok",
                            "bot",
                        ]  # bot is not stand for Robot, It's stand for Back of Thailand.
    ...
    def fetch_all(self) -> NewsFetchResult:
        ...
        for cat_key, cat_meta in NEWS_CATEGORIES.items():
            try:
                ...
                total += len(articles)
                logger.info(
                    f"  [{cat_key}] {len(articles)} articles "
        ...
        return NewsFetchResult(
            fetched_at=get_thai_time().isoformat() + "Z",
...
if __name__ == "__main__":
    ...
    fetcher = GoldNewsFetcher(
        max_per_category=2
    )  # Small count because scraping takes time
