...
def resolve_session_gate(
    ...
    *,
    ...
    if quota_urgent:
        ...
        suggested = 0.55
        notes.append(
            f"Near session end (~{mins_left} min left) — Quota mode: focus short-term momentum; "
            f"confidence hint ≥ {suggested}."
        )
    else:
        llm_mode = "edge"
        suggested = 0.70
        notes.append(
            f"Edge mode: full technical + news context; confidence hint ≥ {suggested}."
        )
