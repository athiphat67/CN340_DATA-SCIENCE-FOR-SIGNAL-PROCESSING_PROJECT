# 📈 CN240 DATA SCIENCE FOR SIGNAL PROCESSING PROJECT

> **Course:** CN240 Data Science for Signal Processing  
> **Institution:** Department of Computer Engineering, Thammasat University  
> **Lecturer:** Professor Dr. Charturong Tantibundhit  

---

## 📝 Project Description
*(ใส่ข้อความ Project Description ที่เลือกไว้ตรงนี้ได้เลยครับ)*

---

## 🔄 Project Phases

กระบวนการดำเนินงานของโปรเจกต์นี้อ้างอิงตามขั้นตอนการทำ Data Science Projects ซึ่งแบ่งออกเป็น 6 เฟสหลัก ดังนี้:

| Phase | Name | Description (รายละเอียดงาน) | Status | Links |
| :---: | :--- | :--- | :---: | :--- |
| **1** | **Discovery** | เป็นช่วงเริ่มต้นเพื่อทำความเข้าใจโดเมนเป้าหมาย ตีกรอบปัญหา และประเมินความพร้อมของทรัพยากร| 🚧 In Progress | [📊 Presentation Slides](#) |
| **2** | **Data Preparation** | การเตรียมโครงสร้างพื้นฐาน ดึงข้อมูลและแปลงข้อมูลแบบ ETL/ELT รวมถึงการทำความเข้าใจข้อมูลเชิงลึก  | ⏳ Todo | - |
| **3** | **Model Planning** | การวางแผนและออกแบบว่าจะใช้โมเดลประเภทไหนให้เหมาะสมกับโจทย์และข้อมูลที่มี | ⏳ Todo | - |
| **4** | **Model Building** | การลงมือเขียนโค้ดเพื่อสร้างโมเดล ฝึกสอน (Train) และประเมินโมเดลเบื้องต้น | ⏳ Todo | - |
| **5** | **Communication of Results** | การนำเสนอผลลัพธ์สุดท้ายที่ได้จากโมเดล เพื่อดูว่าบรรลุเป้าหมายความสำเร็จของโปรเจกต์หรือไม่ | ⏳ Todo | - |
| **6** | **Operationalization** | การนำแนวทางไปใช้งานจริง (Production) หรือการเริ่มทำ Pilot study เพื่อทดสอบประสิทธิภาพในสภาพแวดล้อมจริง  | ⏳ Todo | - |

---

## 📌 Project Tracking

ทีมของเราบริหารจัดการและติดตามความคืบหน้าของโปรเจกต์ (Task Management) ผ่าน **GitHub Projects** ในรูปแบบกระดาน Kanban โดยแยกกระดานตามรอบการทำงาน (Iteration) เพื่อให้บอร์ดสะอาดและโฟกัสเป้าหมายได้ชัดเจนที่สุด

| Iteration | Phases Covered | GitHub Project (Kanban Board) | Status |
| :---: | :--- | :--- | :---: |
| **1** | Phase 1: Discovery<br>Phase 2: Data Preparation | [🔗 CN240 - ML Gold Prediction (Iteration 1)](https://github.com/users/athiphat67/projects/4) | 🟢 Active |
| **2** | Phase 3: Model Planning | [🔗 CN240 - ML Gold Prediction (Iteration 2)](#) | ⚪ Upcoming |
| **3** | Phase 4: Model Building | [🔗 CN240 - ML Gold Prediction (Iteration 3)](#) | ⚪ Upcoming |
| **4** | Phase 5: ECommunication of Results | [🔗 CN240 - ML Gold Prediction (Iteration 4)](#) | ⚪ Upcoming |
| **5** | Phase 6: Operationalization | [🔗 CN240 - ML Gold Prediction (Iteration 5)](#) | ⚪ Upcoming |

**กระบวนการติดตามงานในบอร์ด (Workflow):**
* **📋 Todo:** รวบรวมรายการงาน (Issues) ทั้งหมดที่ต้องทำใน Iteration นั้นๆ
* **🚧 In Progress:** งานที่สมาชิกกำลังดำเนินการอยู่ (ต้อง Assign ชื่อผู้รับผิดชอบเสมอ)
* **👀 In Review:** งานที่ทำเสร็จแล้วและส่งเข้ามาเพื่อให้ทีมหรือ Project Manager ช่วยรีวิวตรวจสอบ
* **✅ Done:** งานที่ผ่านการตรวจสอบและเสร็จสมบูรณ์เรียบร้อย
  
---

## 👥 Team Members 

| No. | Student ID | Name  |
| :---: | :---: | :--- |
| 1 | 6710615292 | อธิภัทร ศูนย์สิทธิ์ |
| 2 | 6710615185 | ภูริช อัมพะวา |
| 3 | 6710685014 | ธีภพ รัตนทรัพย์ศิริ |
| 4 | 6710615060 | โชติวิชช์ ดังสะท้าน |
| 5 | 6710545010 | นพัตธีรา เหลาเกิ้มหุ่ง |
| 6 | 6710625028 | เบญจพล ปินะกะสา |
| 7 | 6710615243 | ลลิตา ทัศนอนันชัย |
| 8 | 6710685055 | พัชรพล มาลัยศรี |
| 9 | 6710615284 | สิทธิพงษ์ คำงาม |
| 10 | 6710615144 | ปณิธาน ตันตื้อ |
